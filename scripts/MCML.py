...
class bMCML(MCML):
	...
	def getDist(self, embed, outLab, inLab, p):
		"""
		...
		for i in outs:
			...
			for j in ins:
				...
				if sub_i.size()[0] > 1:
					...
					d = self.pairwise_dists(sub_i,sub_i,p=p)
	...
	def lossFunc(self, recon_batch, X_b, Yout_b, Yin_b,p):
		"""
		Parameters:
		recon_batch : Reconstruction from decoder for mini-batch
		X_b : Mini-batch of X
	...
	def fit(self, X, Yout, Yin, silent = False, ret_loss = False, p=1.0):
		"""
		Parameters:
		...
		for e in range(self.epochs):
			...
			with torch.autograd.set_detect_anomaly(True):
				for b in range(iters_per_epoch):
					...
					optimizer.zero_grad()
					recon_batch, z = model(X_b)
					losses = self.lossFunc(recon_batch, X_b, Yout_b, Yin_b,p) #*****
	...
	def trainTest(self,X,Yout, Yin, trainFrac = 0.8, silent = False,p=1.0):
		"""
		Parameters:
		X : Input data as numpy array (obs x features)
	...
	def test(self, model, X, Yout, Yin, silent = False,p=1.0):
